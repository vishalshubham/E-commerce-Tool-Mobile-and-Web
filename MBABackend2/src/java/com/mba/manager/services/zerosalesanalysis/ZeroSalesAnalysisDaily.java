 ...
 * @author VISHAL
 */
public class ZeroSalesAnalysisDaily
{
    public static String zeroSalesAnalysisDailyResult = "";
...
zeroSalesAnalysisDailyResult = "";
        if(dBConnection != null)
        {
            try
            {
                System.out.println(" I am in Zero Sales Analysis daily Servlet");
                ...
                ts.setRequestType(32);

                String dd = ts.getTransaction_date_time();


                System.out.println(requestDataMap.get("dailydate")+": daily date");

                ResultSet resultset = dBConnection.runQuery("Fetch_Zero_Sales_Analysis_Daily", new String[]{requestDataMap.get("dailydate")});

                while(resultset.next())
                {
                    zeroSalesAnalysisDailyResult = zeroSalesAnalysisDailyResult + resultset.getString("itemid") + "-" + resultset.getString("item_name") + "-" + resultset.getString("category") + "," ;
                }

                System.out.println("kkkkkkkkkkkkkkkkkkkkkkkkkkk" + zeroSalesAnalysisDailyResult);
