 ...
 * @author VISHAL
 */
public class AddressUpdater extends HttpServlet {
        ...
        if(Integer.parseInt(reqsrc) == 1)
        {
            if(dBConnection != null)
            {
                try
                {
                    ...
                    System.out.print("Address Updater" + address +city+state+pincode+ " _______ " + username);
                    response.reset();
                    response.resetBuffer();
                    boolean rs = dBConnection.runUpdateQuery("Update_User_Address", new String[]{address,city,state,pincode,username});
                    if(rs==true)
                    {
